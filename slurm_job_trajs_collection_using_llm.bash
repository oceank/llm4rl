...
map_name=$1
llm_model_name=$2
n_episodes_to_collect=$3
full_map_observable=$4
use_chatgpt=$5

script="/work/suj/llm4rl/trajs_collection_using_llm.py"

config="--llm_model_name ${llm_model_name} --map_name ${map_name} --n_episodes_to_collect ${n_episodes_to_collect}"
if [ "${full_map_observable}" == 1 ];
then
    config="${config} --full_map_observable"
fi
if [ "${use_chatgpt}" == 1 ];
then
    config="${config} --use_chatgpt"
fi


echo "===> Trajectory collection: ${config}"
